...
/*-----------------------------------------------------------*/
	...
	{
	BaseType_t xIndex;
	ETH_DMADESCTypeDef *pxDescriptor = DMARxDscrTab;
	ETH_DMADESCTypeDef *xLastDescriptor = DMARxDscrTab + ETH_RXBUFNB;

		/* Fill each pxDescriptor descriptor with the right values */
		for( xIndex = 0; pxDescriptor < xLastDescriptor; xIndex++, pxDescriptor++ )
		{
			/* Set Buffer1 size and Second Address Chained bit */
			pxDescriptor->ControlBufferSize = ETH_DMARxDesc_RCH | (uint32_t)ETH_RX_BUF_SIZE;  
			...
			{
				pxDescriptor->Buffer1Addr = (uint32_t)pxBuffer->pucEthernetBuffer;
				pxDescriptor->Status |= ETH_DMARxDesc_OWN;
			}
		}
	}
#endif /* ipconfigZERO_COPY_RX_DRIVER */
...
BaseType_t xResult;
	if( xEMACTaskHandle == NULL )
	{
		#if( ipconfigZERO_COPY_TX_DRIVER != 0 )
		{
			...
			{
				xTXDescriptorSemaphore = xSemaphoreCreateCounting( ( UBaseType_t ) ETH_TXBUFNB - 1, ( UBaseType_t ) ETH_TXBUFNB - 1 );
...
#if( ipconfigZERO_COPY_TX_DRIVER != 0 )
	const TickType_t xBlockTimeTicks = pdMS_TO_TICKS( 1000u );
#endif
...
uint8_t *pucBuffer;
	...
	if( xReceivedLength > 0ul )
	{
		...
		configASSERT( DMA_RX_FRAME_infos->Seg_Count == 1 );
		configASSERT( xReceivedLength <= ETH_RX_BUF_SIZE );

		pucBuffer = ( uint8_t * )frame.buffer;
	...
	if( ( xForce != pdFALSE ) || ( ( ulPHYLinkStatus & BMSR_LINK_STATUS ) != 0 ) )
	{
		...
		if( ETH_InitStructure.ETH_AutoNegotiation != ETH_AutoNegotiation_Disable )
		{
			...
			if( ulTimeout < PHY_READ_TO )
			{
				...
				uint32_t ulControlStatus = ( uint32_t )ETH_ReadPHYRegister( DP83848_PHY_ADDRESS, PHY_REG_1F_PHYSPCS );
...
#endif
UBaseType_t uxLastFreeHeapSpace = 0;
