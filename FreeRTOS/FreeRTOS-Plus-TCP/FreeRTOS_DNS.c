...
#define dnsENTRY_LOOKUP						1
#define dnsENTRY_STORE						2
#define dnsENTRY_PURGE						3
 ...
 */
	...
	static void prvProcessDNSCache( const char *pcName, uint32_t *pulIP, BaseType_t xAction );
...
/*-----------------------------------------------------------*/
	...
	uint32_t ulIPAddress = 0UL;
		prvProcessDNSCache( pcHostName, &ulIPAddress, dnsENTRY_LOOKUP );
		return ulIPAddress;
	}
#endif /* ipconfigUSE_DNS_CACHE == 1 */
/*-----------------------------------------------------------*/
	...
	{
	uint32_t ulIPAddress = 0UL;
		prvProcessDNSCache( pcHostName, &ulIPAddress, dnsENTRY_PURGE );
		return ulIPAddress;
	}
#endif /* ipconfigUSE_DNS_CACHE == 1 */
/*-----------------------------------------------------------*/
			...
			for( x = 0; x < pxDNSMessageHeader->usAnswers; x++ )
			{
				...
				if( usChar2u16( pucByte ) == dnsTYPE_A_HOST )
				{
					...
					if( ( size_t ) *pucByte == sizeof( uint32_t ) )
					{
						...
						#if( ipconfigUSE_DNS_CACHE == 1 )
						{
							prvProcessDNSCache( pcName, &ulIPAddress, dnsENTRY_STORE );
						}
						#endif /* ipconfigUSE_DNS_CACHE */
						#if( ipconfigDNS_USE_CALLBACKS != 0 )
						{
		...
		if( ( usFlags & dnsNBNS_FLAGS_OPCODE_MASK ) == dnsNBNS_FLAGS_OPCODE_QUERY )
		{
			...
			#if( ipconfigUSE_DNS_CACHE == 1 )
			{
				if( ( usFlags & dnsNBNS_FLAGS_RESPONSE ) != 0 )
				{
					...
					prvProcessDNSCache( ( char * ) ucNBNSName, &ulIPAddress, dnsENTRY_STORE );
				}
			}
			#else
			{
...
/*-----------------------------------------------------------*/
	...
	BaseType_t xFreeEntry = -1;
	static BaseType_t xNextEntry = 0;

		if( xAction != dnsENTRY_STORE )
		{
			/* Function is called to look-up or to clear an entry. */
			*pulIP = 0;
		}
		...
		for( x = 0; x < ipconfigDNS_CACHE_ENTRIES; x++ )
		{
			...
			{
				/* Remember the index of a free entry. */
				xFreeEntry = x;
			}
		}
		...
		if( xAction == dnsENTRY_STORE )
		{
			...
			BaseType_t xUseEntry;
				...
				if( xFreeEntry >= 0 )
				{
					/* Use an entry that is not in use. */
					xUseEntry = xFreeEntry;
				}
				else
				{
					/* Overwrite the oldest entry. */
					xUseEntry = xNextEntry;
					xNextEntry++;
					if( xNextEntry == ipconfigDNS_CACHE_ENTRIES )
					{
						xNextEntry = 0;
					}
				}
				...
				strncpy( xDNSCache[ xUseEntry ].pcName, pcName, sizeof( xDNSCache[ xUseEntry ].pcName ) );
				xDNSCache[ xUseEntry ].ulIPAddress = *pulIP;
			}

			if( *pulIP != 0 )
			{
				FreeRTOS_debug_printf( ( "prvProcessDNSCache: add: '%s' @ %lxip\n", pcName, FreeRTOS_ntohl( *pulIP ) ) );
			}
		}
	}
